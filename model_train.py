...
for length in lengths:
    ...
    for fold_id, (train_index, test_index) in enumerate(rskf.split(X, y)):
        ...
        model.compile(optimizer='adam',
                      ...
                      metrics='accuracy')
        model_history = model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=4000, batch_size=128)
    ...
    df.to_csv('results_initial_' + length + '.csv', header=results_labels, index=False)
...
for length in lengths:
    ...
    df = pd.DataFrame(results)
    df.to_csv('results_middle_' + length + '.csv', header=results_labels, index=False)
...
for length in lengths:
    ...
    df = pd.DataFrame(results)
    df.to_csv('results_final' + length + '.csv', header=results_labels, index=False)
